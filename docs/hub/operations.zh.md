...
Queue 满载时先保持 Node Agent 服务运行并恢复 Hub WSS 路径。Node Agent 会在重连后先分页重放已有 Journal，并为控制记录保留容量；不要删除节点数据库、私钥或 Connector Secret，也不要通过重新注册来跳过序列状态。若页面报告 Stream 中断，等待 Runtime 基线重新同步，再重新打开终端等瞬时 Stream；已索引会话在此期间仍会按工作目录显示为离线。

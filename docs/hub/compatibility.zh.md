...
| `0.1.5-rc.2` | Remote 网关和 Session API 已变化 | 已覆盖 Remote 适配、事件桥接和 `$events/result` 回答路径；内置 Web UI 仍是 rc.7 组合 |
| `0.1.6-alpha.2` | 最新预发布版，包含插件管理器和 Session 变化 | 已覆盖 Remote 适配、工具/Skill 路由、问题/取消、事件桥接和 `$events/result` 回答路径；内置 Web UI 仍是 rc.7 组合 |

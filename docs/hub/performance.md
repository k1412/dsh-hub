...
The Connector test fills all four bulk slots, then concurrently submits Goal creation, Goal control, a Settings mutation, and a human response. Every control must return before any bulk slot is released. Browser release verification also checks the production CSP, official directory flow, desktop layout, and a 390 × 844 viewport with an overlay sidebar, bounded Runtime picker, full-screen Settings, and no horizontal overflow.
...
The “Link performance” block under Settings → Hub nodes shows p50, p95, maximum latency, dispatch p95, node-wait p95, error and timeout counts, and response bytes for the latest 2,048 node-backed requests in the current Hub process, including chat/Settings Web APIs and Node Agent controls such as plugins. It groups the window by node, Runtime, and method; the authenticated operator can read the same data from `GET /hub/v1/performance`. This is a bounded in-memory window, cleared on Hub restart, and it retains no request payload, session ID, or response text. Its purpose is to separate Hub dispatch, node waiting, and oversized responses rather than replace a long-term monitoring database.

Browser event writes are ordered and bounded to 512 unsent messages or 4 MiB. When a slow browser crosses either limit, Hub closes the reconstructible stream with a retryable error so official Web resynchronizes from node-authoritative state instead of retaining an unbounded token/event backlog that eventually crashes the page or Hub.

...
- **Settings target** shows the selected node and Runtime in the official Settings header. Switching it updates the tab-local owner and refreshes every live Host-backed Settings scope in place, without remounting the official Settings shell; generation fencing prevents a late response from the previous node from being displayed or written. A target change from the new-session row uses the same isolation path.

...
- **Settings target** shows the selected node and Runtime in the official Settings header. Switching it reloads the official Settings shell against that owner. A target change from the new-session row also invalidates every live Host-backed Settings scope before it can display or write values from the previous node.

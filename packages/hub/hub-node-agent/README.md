...
After reconnecting, Node Agent sends the existing outbox in sequence-number pages before publishing runtime baselines. At the outbox high-water mark it reserves capacity for control records such as command results, runtime lifecycle, and acknowledgements, suppresses stream frames, and requests a new authoritative baseline from affected runtimes after pressure recovers. Connection callbacks and a full queue cause backoff and diagnostics instead of terminating the long-running process.
...
- Suppressed transient stream frames are not presented as reliable replay. Reconstructible state returns from a runtime baseline, while transient output such as a terminal is explicitly reported as interrupted.

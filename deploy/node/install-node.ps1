...
try {
  ...
  & $AgentExecutable init --hub $HubUrl --node $NodeId --access-client-id $AccessClientId --profile $ProfileName --runtime-id default --profile-directory $ProfileDirectory --dsh-executable $DshExecutable --install-connector (Join-Path $TemporaryDirectory $ConnectorAsset)

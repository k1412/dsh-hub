...
The new-session surface does not require the operator to remember an absolute path on every machine. It reads the selected runtime's current working directory through the version-negotiated, audited `dsh.runtime.health` operation and auto-fills it, then browses node directories on demand through `dsh.files.list`. The operator can still type a complete path. Directory results live only in the current browser dialog state; Hub does not write them to the session index, database, or object directory. Changing runtimes discards the prior node's directory state, and stale responses cannot overwrite the current selection. Session baselines already contain each session's working directory; Hub indexes that minimal metadata beside the title and renders sidebar project groups keyed by `{nodeId, runtimeId, workspacePath}`. Equal directory names never merge across nodes or runtimes, and sessions without a working directory enter a separate fallback group.

## Storage and fleet operations

The reference deployment uses SQLite in WAL mode plus a content-addressed directory. SQLite stores node identity, runtime and capability metadata, a minimal session index containing title and working directory, command delivery state, and audit entries. Schema v2 adds a nullable `workspace_path` column to an existing v1 session index in place; the migration retains every row and the next node baseline fills its path. The object directory accepts only immutable artifacts explicitly imported by the operator and is not populated by ordinary node flows. Node Agent stores downloaded plugin artifacts, rollback transactions, and snapshots in owner-only local state. Hub does not transparently cache conversation bodies, workspace content, terminal output, credentials, or general node files.
...
Hub Web interaction tests also pin explicit new-session dialog close semantics, lossless compound runtime identifiers, parent-directory boundaries for POSIX, Windows, and UNC paths, filtering of node directory results, and visible state during creation. A browser API test pins the directory-list command to the capability version advertised by the node and explicitly acknowledges its one-time result after reading it. Project-projection tests cover grouping identity, ordering, Windows paths, the pathless fallback, HTML escaping, and click selection shared by the mobile layout. Storage and server tests cover the row-preserving v1-to-v2 migration, Connector baselines, WSS projection, and REST output. A cancel action therefore cannot submit, and the HTML parser cannot rewrite a runtime delimiter and cause creation to return silently.

...
The Hub control contract is owned under `/hub/v1`. JSON REST carries bounded requests and mutations, a resumable SSE channel carries reconstructible control events, and a dedicated same-origin WebSocket relays interactive PTY frames. The Hub Web build uses the official DSH client roster directly. Its `/api/*` requests and official event WebSockets carry the selected `{nodeId, runtimeId}` through Hub and the `dsh.web` capability to the Connector's Host services. The upstream Web API remains a browser-to-runtime interface rather than the Hub-to-node protocol.

Official client packages continue to own conversation, workspace, project grouping, directory selection, streaming, scrolling, errors, and mobile behavior. Hub adds only reviewed Settings contributions for enrollment, fleet state, runtime switching, plugin health, update history, rollback, explicit snapshots, and acknowledged rescue diagnostics. Node selection changes the browser target without replacing the official interaction model or loading JavaScript supplied by a node.
...
The Hub and Node Agent remain security-sensitive because enrollment grants broad authority. Operators must protect the Hub origin, Node Agent account, identity files, service tokens, backups, and artifact approval process. Offline nodes expose only indexed metadata until they reconnect, and every capability or rich UI feature requires an explicitly versioned adapter and reviewed Hub surface. Hub backups and Node Agent state backups are separate: plugin rollback and snapshots survive only while their node-local material is retained. Adopting a new official Web roster requires rerunning the assembled Hub build and official browser regression inventory.

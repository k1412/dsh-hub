window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region \0dsh-css:/home/wuyang/Code/deepcode/dsh-hub/packages/client/ui-goal/src/client/GoalBar.module.css.mjs
		const css$1 = ".EPH8Aq_dock{box-sizing:border-box;width:calc(100% - var(--dsh-composer-side-clearance) - var(--dsh-composer-side-clearance) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset));margin:0 auto}.EPH8Aq_bar{box-sizing:border-box;width:100%;max-width:calc(var(--dsh-composer-card-max-width) - 4 * var(--dsh-composer-dock-inset));border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-specific-tip);border-radius:12px;align-items:center;gap:10px;height:36px;margin:0 auto;padding:4px 5px 4px 12px;display:flex}.EPH8Aq_goalGlyph{color:var(--dsw-alias-label-tertiary);flex:none;display:inline-flex}.EPH8Aq_label{color:var(--dsw-alias-label-primary);flex:none;font-size:13px;font-weight:500;line-height:24px}.EPH8Aq_objective{min-width:0;color:var(--dsw-alias-label-primary-dimmed);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:13px;line-height:20px;overflow:hidden}.EPH8Aq_error{min-width:0;color:var(--dsw-alias-state-error-primary);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:12px;line-height:20px;overflow:hidden}.EPH8Aq_objectiveInput{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;height:26px;color:var(--dsw-alias-label-primary);border-radius:6px;outline:none;flex:1;padding:0 8px;font-size:13px;line-height:20px}.EPH8Aq_objectiveInput:focus{border-color:var(--dsw-alias-state-business-primary)}.EPH8Aq_objectiveInput::placeholder{color:var(--dsw-alias-label-caption)}.EPH8Aq_actions{flex:none;align-items:center;gap:10px;display:flex}.EPH8Aq_iconBtn{width:28px;height:28px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:999px;justify-content:center;align-items:center;padding:0;display:inline-flex}.EPH8Aq_iconBtn:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary)}.EPH8Aq_iconBtn:disabled{opacity:.4;cursor:default}";
		...
		var GoalBar_module_css_default = {
			"iconBtn": "EPH8Aq_iconBtn",
			"goalGlyph": "EPH8Aq_goalGlyph",
			"dock": "EPH8Aq_dock",
			"objective": "EPH8Aq_objective",
			"error": "EPH8Aq_error",
			"label": "EPH8Aq_label",
			"bar": "EPH8Aq_bar",
			"objectiveInput": "EPH8Aq_objectiveInput",
			"actions": "EPH8Aq_actions"
		};
		//#endregion
		...
		*/
		...
		function GoalBar({ goal, onEdit, onPause, onResume, onClear, t }) {
			...
			const runAction = (0, react.useCallback)(async (action) => {
				...
				try {
					...
					setActionError(`${message} (transport)`);
					return {
						ok: false,
						error: {
							code: "transport",
			...
			const blockedReason = goal.phase === "blocked" ? goal.blockedReason?.message : void 0;
			return (0, react_jsx_runtime.jsx)("div", {
				className: GoalBar_module_css_default.dock,
				"data-goal-bar": true,
				children: (0, react_jsx_runtime.jsxs)("div", {
					className: GoalBar_module_css_default.bar,
					title: blockedReason === void 0 ? void 0 : goal.objective,
					children: [
						...
						(0, react_jsx_runtime.jsx)("span", {
							...
							children: blockedReason ?? goal.objective
						}),
						actionError !== null && (0, react_jsx_runtime.jsx)("span", {
							className: GoalBar_module_css_default.error,
							role: "alert",
		...
		//#endregion
		//#region \0dsh-css:/home/wuyang/Code/deepcode/dsh-hub/packages/client/ui-goal/src/client/GoalCommandInputView.module.css.mjs
		const css = ".IqiFKW_row{flex-direction:column;align-items:flex-end;gap:6px;display:flex}.IqiFKW_stack{flex-direction:column;align-items:flex-end;min-width:0;max-width:min(525px,82%);display:flex}.IqiFKW_bubble{overflow-wrap:anywhere;background:var(--dsw-specific-bubble);max-width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-markdown-code);white-space:pre-wrap;border-radius:22px;padding:10px 16px}";
		...
		*/
		function apply(ctx) {
			...
			*/
			...
			const mutate = async (sessionId, action) => {
				try {
					const result = await action();
